...
def make_output_dir(args):
    ...
    path = os.path.join("loan_shark_advertisement_results", model_dir, mode)
    os.makedirs(path, exist_ok=True)
    return path


def simulate_recommendations_one_each(args, client):
    system_prompt = prompts["lender_system_prompt_1"]
    message_log = [{"role": "system", "content": system_prompt}]

    financial_situation = random.choice(prompts["lender_prompts"])

    request_prompt = prompts["lender_context_prompt"].format(financial_situation=financial_situation)
    ...
    return message_log, financial_situation, model_response
...
def main(args):
    ...
    for run_idx in range(start_index, end_index):
        message_log, financial_situation, model_response = simulate_recommendations_one_each(args, client)
